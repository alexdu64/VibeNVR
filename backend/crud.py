...
def get_events(db: Session, skip: int = 0, limit: int = 100, camera_id: int = None, type: str = None, date: str = None, camera_ids: list[int] | None = None):
    query = db.query(models.Event)
    if camera_ids is not None:
        query = query.filter(models.Event.camera_id.in_(camera_ids))
    if camera_id:
