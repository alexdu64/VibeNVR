...
def migrate():
    ...
    # Camera Permissions (Fallback creation)
    with engine.connect() as conn:
        try:
            conn.execute(text("SELECT 1 FROM camera_permissions LIMIT 1"))
        except:
            logger.info("Creating camera_permissions table via migration...")
            models.CameraPermission.__table__.create(engine)
            conn.commit()
            logger.info("camera_permissions table created.")
