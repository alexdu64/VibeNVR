...
import json
import schemas, models, permissions
...
async def ptz_move(
    ...
    if not permissions.user_can_control_camera(current_user, camera_id, db):
...
async def get_ptz_presets(
    ...
    if not camera:
        raise HTTPException(status_code=404, detail="Camera not found")
    if not permissions.user_can_access_camera(current_user, camera_id, db):
        raise HTTPException(status_code=403, detail="Access denied")
