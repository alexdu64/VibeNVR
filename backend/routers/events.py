...
import crud, schemas, database, os, requests, threading, models, subprocess, auth_service, permissions
...
def read_events(
    ...
    if user.role == "admin":
        ...
        accessible_ids = permissions.get_accessible_camera_ids_for_replay(user, db)
        if not accessible_ids:
            return []
        if camera_id is not None and camera_id not in accessible_ids:
            return []
        events = crud.get_events(db, skip=skip, limit=limit, camera_id=camera_id, type=type, date=date, camera_ids=accessible_ids)
    return events
...
async def download_event(event_id: int, request: Request, token: Optional[str] = None):
    ...
    with database.get_db_ctx() as db:
        ...
        if not event:
            raise HTTPException(status_code=404, detail="Event not found")
        if not permissions.user_can_replay_camera(user, event.camera_id, db):
