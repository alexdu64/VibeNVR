...
export const LiveView = () => {
    ...
    useEffect(() => {
        ...
        fetch(`${API_BASE}/users/${user.id}/camera-permissions`, {
            headers: { Authorization: `Bearer ${token}` }
        })
            .then(res => res.json())
            .then(data => {
                ...
                const map = {};
                data.forEach(p => { map[p.camera_id] = p; });
                setUserPermissions(map);
            })
            .catch(err => console.error('Failed to fetch camera permissions', err));
    }, [token, user]);
                                            ...
                                            <div className={`grid gap-2 sm:gap-4 ${columnSetting === 'auto' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' :
                                                ...
                                                {grouped[groupName].map((cam, i) => (
                                                    <VideoPlayer
                                                        ...
                                                        canControl={userPermissions[cam.id]?.can_control ?? false}
                                                    />
                                                ))}
                                            </div>
                                        </div>
